...
QSkinWidgetResizeHandler::QSkinWidgetResizeHandler(QSkinObject * skobj, QWidget *parent, QWidget *cw)
    : QObject(parent), widget(parent), childWidget(cw ? cw : parent),
      fw(0), extrahei(0), buttonDown(false), moveResizeMode(false), sizeprotect(false), movingEnabled(true)
{
	obj = skobj;
	widgetType = 0;
    mode = Nowhere;
...
bool QSkinWidgetResizeHandler::eventFilter(QObject *o, QEvent *ee)
{
    ...
    case QEvent::MouseButtonRelease:
        ...
        if (e->button() == Qt::LeftButton)
        {
            ...
            if (mode == Center)
            {
                if (movingEnabled)
                    return true;
               // widget->mousePressEvent(static_cast<QMouseEvent*>(ee));
            }
            else
            {
               // widget->mousePressEvent(static_cast<QMouseEvent*>(ee));
    ...
    case QEvent::MouseMove:
    {
        ...
        mouseMoveEvent(e);
        movingEnabled = me;
        obj->mouseMoveEvent(static_cast<QMouseEvent*>(ee));
        if (mode == Center)
        {
            if (movingEnabled)
                return true;
