...
class RepeatButton : public QStyledItemDelegate {
    Q_OBJECT
public:

    explicit RepeatButton(QWidget *parent = 0,
    ...
    /*    if((!index.parent().isValid())&& (option.state & QStyle::State_MouseOver))
        ...
        else */if(!index.parent().isValid()&& index.column() == 6)
        {
            painter->drawPixmap(closeIconPos(option), m_closeIcon);
        }
...
private:
    QPixmap m_closeIcon,m_HoverIcon;
    static const int margin = 0; // pixels to keep arount the icon
    bool hover;

    Q_DISABLE_COPY(RepeatButton)
};
